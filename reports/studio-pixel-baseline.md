...
更新时间：2026-07-21 20:15 Asia/Shanghai

## 证据边界

- 2026-07-21 公开抓取：`https://studio.dreem-world.ai/worlds` 仅返回 SPA 标题；同日浏览器核验根路径和 `/worlds` 均在展示产品内容前重定向到 `/sign-in`。
- 登录墙可确认左右分栏结构、插画区、OAuth/邮箱登录区；无法确认登录后的世界卡片、详情标签或故事控件。
...
| 范围 | 本地路由 | 已确认基线 |
| --- | --- | --- |
| 登录入口 | `/login` | 轻量推荐码壳；左右分栏；不臆造服务端认证、登录后控件或推荐码校验结果 |
| 世界列表 | `/world-builder/worlds` | 只展示用户项目，不强制混入样例；炭黑舞台、低对比边框、克制玫瑰操作色；标题使用 Instrument Serif，正文使用 Public Sans |
