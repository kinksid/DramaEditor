...
export function GraphToolbar({ onPreview }: Props) {
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 bg-white px-4 py-3 shadow-soft">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={() => setIssues(currentIssues)} className="inline-flex items-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-3 py-2 text-sm font-medium text-orange-700">
        ...
        <button onClick={handleGenerateVideos} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50">
          <Wand2 size={16} /> 生成全部视频
        </button>
        <button onClick={() => addSceneNode()} className="inline-flex items-center gap-2 rounded-xl bg-ink px-3 py-2 text-sm font-medium text-white">
          <Plus size={16} /> 添加视频
        </button>
        <button onClick={() => addInteractionNode()} className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-medium text-white">
          <Plus size={16} /> 添加互动
        </button>
        <button onClick={() => addEndingNode()} className="inline-flex items-center gap-2 rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-sm font-medium text-red-600">
          <Flag size={16} /> 添加结局
        </button>
        ...
        <button onClick={downloadJson} className="grid size-10 place-items-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50" title="导出故事数据">
          <Download size={17} />
        </button>
        ...
        <button onClick={() => copyWorkflow("libtv")} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50" title="复制 LibTV 工作流">
          <ClipboardCopy size={16} /> LibTV
        </button>
                ...
                issues.map((issue) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className={issue.severity === "error" ? "text-red-600" : "text-orange-600"}>
