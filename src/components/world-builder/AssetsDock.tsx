...
import { MODAL_DRAWER, MODAL_OVERLAY_70, MODAL_SCRIM } from "@/lib/modalTheme";
...
export function AssetsDock({ embedded = false, projectId }: AssetsDockProps) {
  ...
  const storyGraphHref = `/world-builder/story-graph?project=${projectId ?? activeProjectId ?? ""}`;
  ...
  return (
    <>
      ...
      <div className={cn("flex min-h-full flex-col text-white", embedded ? "bg-stage rounded-2xl border border-card-border" : "bg-[#0c0a0f]")}>
        ...
        {embedded && (
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-card-border px-5 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <label className="inline-flex items-center gap-2 rounded-xl border border-card-border bg-panel px-3 py-2 text-sm">
              ...
              <Link
                href={storyGraphHref}
                className="inline-flex items-center gap-2 rounded-xl bg-accent px-3 py-2 text-sm font-semibold text-white hover:bg-accent-deep"
              >
                <Film size={14} /> 打开故事图
              </Link>
            </div>
      ...
      {preview && (
        <div
          className={cn(MODAL_OVERLAY_70, "flex items-center justify-center p-6")}
      ...
      {panel && (
        <>
          <div className={cn(MODAL_SCRIM, "z-40")} onClick={() => setPanel(null)} />
          <div className={cn(MODAL_DRAWER, "fixed inset-y-0 right-0 z-50 w-full max-w-lg overflow-y-auto")}>
            <div className="sticky top-0 z-10 flex items-center justify-between border-b border-card-border px-6 py-4">
